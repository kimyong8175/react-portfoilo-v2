            ...
            {projects.map((project, index) => (
              <div key={index} className="sm:w-1/2 w-100 p-4 group">
                <div className="flex relative rounded-md group-hover:text-opacity-100">
                  <img
                    alt="gallery"
                    className="absolute inset-0 w-full h-full object-cover object-center rounded-md group-hover:blur-sm"
                    src={project.image}
                  />
                  <div className="px-8 py-10 relative z-10 w-full  border-2 opacity-0 hover:opacity-100 group-hover:border-none ">
                    <h2 className="tracking-widest text-sm title-font font-medium text-green-400 mb-1 text-green">
                      {project.subtitle}
                    </h2>
